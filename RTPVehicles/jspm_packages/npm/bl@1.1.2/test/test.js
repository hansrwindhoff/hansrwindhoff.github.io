...
(function(Buffer, process) {
  ...
  tape('multiple bytes from crazy nested buffer lists', function(t) {
    var bl = new BufferList();
    bl.append(new BufferList([new BufferList([new BufferList(new Buffer('abc')), new Buffer('d'), new BufferList(new Buffer('efg'))]), new BufferList([new Buffer('hi')]), new BufferList(new Buffer('j'))]));
    t.equal(bl.length, 10);
    t.equal(bl.slice(0, 10).toString('ascii'), 'abcdefghij');
    t.equal(bl.slice(3, 10).toString('ascii'), 'defghij');
    t.equal(bl.slice(3, 6).toString('ascii'), 'def');
  ...
  tape('append chainable (test results)', function(t) {
    var bl = new BufferList('abc').append([new BufferList('def')]).append(new BufferList([new Buffer('ghi'), new BufferList('jkl')])).append([new Buffer('mnop'), new BufferList([new Buffer('qrstu'), new Buffer('vwxyz')])]);
    t.equal(bl.length, 26);
    t.equal(bl.slice().toString('ascii'), 'abcdefghijklmnopqrstuvwxyz');
    t.end();
  });
