...
(function(global) {
  ...
  function validateComplex(test, Game) {
    var Car = Game.Cars.Car,
        ...
        Speed = Car.Speed;
    var vendor;
    var car = new Car("Rusty", vendor = new Vendor({
      "name": "Iron Inc.",
      "address": {"country": "US"}
    }), Speed.SUPERFAST);
  ...
  var suite = {
    ...
    "float": function(test) {
      try {
        var str_proto = "message Float {" + " required float f = 1;" + "}";
    ...
    "options": {
      ...
      "export": function(test) {
        try {
          ...
          test.deepEqual(My.Test.$options, {"(inmessage)": "My.Test"});
        } catch (e) {
          fail(e);
        }
        test.done();
    ...
    "repeated": {
      ...
      "both": function(test) {
        try {
          ...
          var Inner = root.Inner;
          var outer = new Outer({
            inner: [new Inner(1), new Inner(2)],
            innerPacked: [new Inner(3), new Inner(4)]
          });
    ...
    "extend": function(test) {
      try {
        ...
        test.deepEqual(TFoo.extensions, [2, ProtoBuf.Lang.ID_MAX]);
        test.deepEqual(TBar.extensions, [ProtoBuf.Lang.ID_MIN, ProtoBuf.Lang.ID_MAX]);
        var root = builder.build();
    ...
    "services": function(test) {
      try {
        ...
        var root = builder.build(),
            ...
            called = false;
        test.deepEqual(MyService.$options, {"(my_service_option)": "FOO"});
        ...
        var myService = new MyService(function(method, req, callback) {
          ...
          called = true;
          setTimeout(callback.bind(this, null, (new ResponseType()).encode()));
        });
