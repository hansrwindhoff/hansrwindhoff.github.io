...
test("[func] Returns a falsy body for 204 responses", function(assert) {
  xhr({uri: "/mock/no-content"}, function(err, resp, body) {
...
test("[func] Calls the callback once even if error is thrown issue #127", function(assert) {
  ...
  try {
    xhr({uri: "instanterror://foo"}, function(err, resp, body) {
      count++;
...
test("withCredentials option", function(assert) {
  if (!window.XDomainRequest) {
    ...
    assert.ok(!req.withCredentials, "withCredentials not true");
    req = xhr({withCredentials: true}, function() {});
    assert.ok(req.withCredentials, "withCredentials set to true");
  }
  assert.end();
});
test("withCredentials ignored when using synchronous requests", function(assert) {
  if (!window.XDomainRequest) {
    ...
    assert.ok(!req.withCredentials, "sync overrides withCredentials");
  }
  assert.end();
});
test("XDR usage (run on IE8 or 9)", function(assert) {
  ...
  assert.ok(!window.XDomainRequest || window.XDomainRequest === req.constructor, "Uses XDR when told to");
  if (!!window.XDomainRequest) {
...
test("[func] xhr[method] get, put, post, patch", function(assert) {
  ...
  forEach(methods, function(method) {
    xhr[method]({uri: "/mock/200ok"}, function(err, resp, body) {
      i++;
      assert.ifError(err, "no err");
      assert.equal(resp.statusCode, 200);
...
test("xhr[method] get, put, post, patch with url shorthands and options", function(assert) {
  var i = 0;
  forEach(methods, function(method) {
    var req = xhr[method]("/some-test", {headers: {foo: 'bar'}}, function(err, resp, body) {
      i++;
...
if (!window.XDomainRequest) {
  test("[func] xhr.head", function(assert) {
    xhr.head({uri: "/mock/200ok"}, function(err, resp, body) {
      assert.ifError(err, "no err");
      assert.equal(resp.statusCode, 200);
  ...
  test("[func] xhr.del", function(assert) {
    xhr.del({uri: "/mock/200ok"}, function(err, resp, body) {
      assert.ifError(err, "no err");
      assert.equal(resp.statusCode, 200);
...
test("url signature with object", function(assert) {
  xhr("/some-test", {headers: {"foo": "bar"}}, function(err, resp, body) {
    assert.equal(resp.url, '/some-test');
    assert.equal(resp.rawRequest.headers.foo, 'bar');
    assert.end();
  });
