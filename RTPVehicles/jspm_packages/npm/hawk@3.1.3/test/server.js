...
describe('Server', function() {
  ...
  describe('authenticate()', function() {
    ...
    it('errors on an missing host header', function(done) {
      var req = {
        method: 'GET',
        url: '/resource/4?filter=a',
        headers: {authorization: 'Hawk id="123", ts="1353788437", nonce="k3j4h2", mac="/qwS4UjfVWMcUyW6EEgUH4jlr7T/wuKe3dKijvTvSos=", ext="hello"'}
      };
      Hawk.server.authenticate(req, credentialsFunc, {localtimeOffsetMsec: 1353788437000 - Hawk.utils.now()}, function(err, credentials, artifacts) {
        expect(err).to.exist();
  ...
  describe('header()', function() {
    ...
    it('errors on missing artifacts', function(done) {
      ...
      var header = Hawk.server.header(credentials, null, {
        payload: 'some reply',
        contentType: 'text/plain',
        ext: 'response-specific'
      });
    ...
    it('errors on invalid artifacts', function(done) {
      ...
      var header = Hawk.server.header(credentials, 5, {
        payload: 'some reply',
        contentType: 'text/plain',
        ext: 'response-specific'
      });
    ...
    it('errors on missing credentials', function(done) {
      ...
      var header = Hawk.server.header(null, artifacts, {
        payload: 'some reply',
        contentType: 'text/plain',
        ext: 'response-specific'
      });
  ...
  describe('authenticateMessage()', function() {
    ...
    it('errors on nonce collision', function(done) {
      credentialsFunc('123456', function(err, credentials1) {
        var auth = Hawk.client.message('example.com', 8080, 'some message', {credentials: credentials1});
        Hawk.server.authenticateMessage('example.com', 8080, 'some message', auth, credentialsFunc, {nonceFunc: function(key, nonce, ts, nonceCallback) {
            nonceCallback(true);
          }}, function(err, credentials2) {
          expect(err).to.exist();
    ...
    it('should fail authorization on nonce error', function(done) {
      credentialsFunc('123456', function(err, credentials1) {
        var auth = Hawk.client.message('example.com', 8080, 'some message', {credentials: credentials1});
        expect(auth).to.exist();
        Hawk.server.authenticateMessage('example.com', 8080, 'some message', auth, credentialsFunc, {nonceFunc: function(key, nonce, ts, callback) {
